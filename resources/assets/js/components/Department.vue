	 ...
	 <a class="panel-block ">
	  ...
	  <a class="panel-block " v-for="item,key in temp">
	    ...
	    <span class="panel-icon column is-1">
	      <i class="has-text-danger fa fa-trash" aria-hidden="true" @click="del(key,item.department_id)"></i>
	    </span>
	    <span class="panel-icon column is-1">
...
<script>
let Add = require('./DepartmentAdd.vue');
let Show = require('./DepartmentShow.vue');
let Update = require('./DepartmentUpdate.vue');
	export default{
		...
		methods:{
			...
			del(key,department_id){
				if (confirm("Are you sure ?")) {
					this.loading = !this.loading
					axios.delete(`/department/${department_id}`)
					.then((response)=> {this.lists.splice(key,1);this.loading = !this.loading})
					.catch((error) => this.errors = error.response.data.errors)	
				}
				console.log(`${key} ${department_id}`)
			}
		}
	}
</script>